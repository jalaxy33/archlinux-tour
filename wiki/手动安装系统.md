...
上篇：安装archlinux的[准备工作](./安装archlinux#准备工作)
...
1. 查看分区情况
    ...
    确保硬盘的分区表类型是gpt：[检查方法](./安装archlinux/检查分区表类型)
...
2. 用`cfdisk`创建分区
    ...
    - **创建efi启动分区**：
        ...
        如果类型里没有`efi system`的话，说明不是GPT分区表，请按照[这个教程](./安装archlinux/检查分区表类型)修改。
...
`pacstrap`命令把软件安装到指定目录下；`-K`参数表示复制密钥。

- 安装基本包
    ```sh
    pacstrap -K /mnt base base-devel
    ```
    `base`是基本包；`base-devel`是编译其他软件的时候用的

- 安装Linux内核

    > **内核的选择**：要续航装`linux`，要性能装`linux-zen`。推荐多装一个 `linux-lts` 稳定版内核以备不时之需。
...
- 其他必要的包
    ...
    ```sh
    pacstrap -K /mnt linux-firmware btrfs-progs
    ```

    `linux-firmware`是基本固件

    `btrfs-progs`是btrfs文件系统的管理工具
...
- 内核微码，用于修复和优化cpu，根据自己的cpu安装
    ```sh
    pacstrap -K /mnt intel-ucode  #intel用户装这个
    pacstrap -K /mnt amd-ucode  #amd用户装这个
    ```
...
- 设置时区为上海：
    ...
    此时再运行 `timedatectl` 可以看到时区已经变成 `Shanghai`。
...
3. 在grub的默认安装位置创建软链接
    ...
    大多数程序会默认检测`/boot/grub`作为grub的安装位置，所以创建一个链接方便使用。之后对 `/boot/grub` 的操作也能反映到 `/efi/grub` 中。
...
4. GRUB的后续配置请查看：[重启前的工作](./安装后重启前的工作)



### 可选：退出chroot

**先不退出**，后续还有需要在 chroot 里进行的操作。
