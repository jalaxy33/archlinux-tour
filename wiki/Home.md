一些 ArchLinux 安装和配置记录。


参考了这些优秀的教程：
- [ShorinArchExperience-ArchlinuxGuide](https://github.com/SHORiN-KiWATA/ShorinArchExperience-ArchlinuxGuide)：超级详细的安装配置教程。视频教程：
  - [「Archlinux究极指南2025」从手动安装到显卡直通，最后删除Linux](https://www.bilibili.com/video/BV1L2gxzVEgs)
  - [「Linux游戏指南」一次挑战与一场斗争](https://www.bilibili.com/video/BV1zyttzPEmp)
  - [从「Linuxmint入门」到「ArchLinux安装详解」桌面端Linux入门的最佳路径](https://www.bilibili.com/video/BV19DBqB4EY4/)
- [archlinux简明指南](https://arch.icekylin.online/)：查漏补缺
- [archwiki](https://wiki.archlinuxcn.org/wiki/%E9%A6%96%E9%A1%B5)：archlinux官方使用指南，对于很多问题都有很详细的解答。


## 下一步：[安装任意系统前的准备工作](./安装任意系统前的准备工作)


